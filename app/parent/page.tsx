      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <section style={{ background: "#fff", borderRadius: 14, padding: "18px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {profile?.pDevoirsAMaison && (
              <Row label="Comportement devoirs" value={profile.pDevoirsAMaison as string} />
            )}
            {profile?.eConcentration && (
              <Row label="Concentration" value={profile.eConcentration as string} />
            )}
            {profile?.eMomentJournee && (
              <Row label="Meilleur moment de travail" value={profile.eMomentJournee as string} />
            )}
          </div>

          <div style={{ marginTop: 14, padding: "10px 14px", background: C.parchment, borderRadius: 8, fontSize: 13, color: C.warmGray }}>
            💡 Pour modifier le profil de {prenom}, refais l'onboarding depuis l'accueil.
          </div>
        </section>
